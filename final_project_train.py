...
from best_genome_selection import (
    load_top_k_flat, load_top_k_ice, load_top_k_hill,
...
_world_worker: "FinalWorld | None" = None
...
def _get_thread_world() -> FinalWorld:
    if not hasattr(_thread_local, "world"):
        _thread_local.world = FinalWorld()
...
def run_multi_task_evolution(
    ...
    if SINGLE_OBJECTIVE :
        ...
        if initial_population is not None:
            pop0 = np.clip(initial_population[:population_size], bounds[0], bounds[1])
            ea.initialise_x0 = lambda: pop0
    ...
    print(f"  eval mode: {mode_str}  (cores={_cpus})\n")

    pool_cls = ThreadPoolExecutor if IS_MAC else ProcessPoolExecutor
    pool_kw  = {} if IS_MAC else {"initializer": _init_worker}

    t_start = time.time()

    with pool_cls(max_workers=n_workers, **pool_kw) as pool:
        for gen in range(num_generations):
            pop = ea.ask()

            if not BODY_PARAM:
                pop_eval = [np.concatenate([g, frozen_body]) for g in pop]
            else:
                pop_eval = list(pop)

            args = [(g, n_repeats, n_steps) for g in pop_eval]
            results = list(pool.map(_eval_worker, args, chunksize=1))
            ...
            if scalars[best_idx] > _best_scalar:
                ...
                with open(_best_xml_stage, "w") as fh:
                    fh.write(xml_strs[best_idx])

            elapsed = time.time() - t_start
            print(
                f"[Gen {gen+1}/{num_generations}]"
                f"  best_scalar={scalars[best_idx]:.2f}"
...
if __name__ == "__main__":
    ...
    if not VIDEO :
        ...
        initial_pop = build_diverse_initial_population(
            n_params=world_tmp.n_params,
        ...
        run_multi_task_evolution(
            num_generations=2,
            population_size=3*K*12 + 1,  # 128 random + 36 per top-k genome
            n_parents=(3*K*12 + 1)// 2,
            n_repeats=3,
            n_steps=1000,
            ckpt_interval=1,
